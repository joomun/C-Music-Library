...
void MusicLibrary::loadSongsFromFile(const std::string &filename)
{
    ...
    while (getline(file, line))
    {
        ...
        if (!title.empty() && !artist.empty() && !duration_str.empty())
        {
            ...
            try
            {
                duration = std::stoi(duration_str); // convert duration string to integer
                if (duration <= 0)
                {
                    throw std::invalid_argument("invalid duration"); // throw an exception if duration is not positive
                }
            }
            ...
            if (temp_songs.count(key) == 0 && songs_.count(key) == 0 && seen_songs.count(key) == 0)
            { // check if the song is already in the map or has already been loaded
            ...
            else if (temp_songs.count(key) > 0 || songs_.count(key) > 0)
            { // song already exists in the map or temporary map
